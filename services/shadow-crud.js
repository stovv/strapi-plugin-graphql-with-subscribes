...
const buildSingleType = (model, ctx) => {
  ...
  if (model.lifecycles) {
    Object.keys(model.lifecycles).forEach(action => {
      const subscriptionSchema = buildSubscriptionTypeDef({ model, action }, ctx);
...
// - Implement batch methods (need to update the content-manager as well).
// - Implement nested transactional methods (create/update).
const buildSubscriptionTypeDef = ({ model, action }, ctx) => {
  const capitalizedName = _.upperFirst(toSingular(model.modelName));
  const mutationName = `${action}${capitalizedName}`;
  ...
  const resolverOpts = {
    resolver: `${model.uid}.${action}`,
    transformOutput: result => ({ [toSingular(model.modelName)]: result }),
    ..._.get(ctx.schema, `resolver.Subscription.${mutationName}`, {}),
  };
  ...
  // ignore if disabled
  if (!isSubscriptionEnabled(ctx.schema, mutationName)) {
    return {
      definition,
    };
  }
  ...
  switch (action) {
    ...
    case 'afterCountSearch':
      subscriptionDef = `${mutationName}`;
      break;
    default:
      subscriptionDef = `${mutationName}(id: ID!)`;
      break;
  }
  ...
  return {
    definition,
    subscription: {
      [subscriptionDef]: `${mutationName}Payload`,
    },
    resolvers: {
      Subscription: {
        [mutationName]: buildSubscription(mutationName, resolverOpts, model),
      },
    },
  };
};
